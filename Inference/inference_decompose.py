...
def choose_API(API_instruction, API_list, question, thought, chat):
    question = question+f"\nthought: {thought}"
    prompt = choose_API_prompt.format(API_instruction=API_instruction, API_list=API_list, question=question)
    message = [{'role': 'user', 
                 'content': prompt}]
    ind = 0
    while True:
...
# API_instruction = "'suivi-colis' can track packages in Nouvelle-Calédonie.\nThis tool has 4 APIs:\n1. 'Health' can get the API's health.\n2. 'Latest' can retrieve the current status of a package.\n3. 'Count' can count the number of package status updates, useful for limiting network consumption or IoT resources.\n4. 'All' can retrieve the entire package history from its shipment to the current status."
# API_list = [
#             {
#                 "name": "Health",
#                 "description": "Get the API's health",
#                 "Example": {
#                     "Scenario": "Check the health of the API",
#                 }
#             },
#             {
#                 "name": "Latest",
...
#                 "Example": {
#                     "Scenario": "To count the number of steps in the history of the package with ID CA107308006SI",
#                 }
#             },
#             {
#                 "name": "All",
...
#                 "Example": {
#                     "Scenario": "if you want to get the complete history of a package with ID CA107308006SI",
#                 }
#             }
# ]
...
# ls = choose_API(API_instruction, API_list, question, thought, chat)
# print(ls)
#response: {"name": "Latest"}
...
def inference(query, relevant_APIs, api_list, white_list, tool_doc, tool_dic, chat, restart_num=5, max_step=3):
    ...
    tool_dic_ids = []
    for ele in tool_dic:
        tool_dic_ids.append(str(ele["ID"]))
    restart_time = 0
    step_num = 0
    ...
    while True:
        ...
        for ele in tool_dic:
            ...
            if ID in tool_used:
                continue
            des = ele['Description']
            tool_list.append({"ID": ID, "Description": des})
        ...
        try:
            ...
            tool_id = int(tool_id)
            tool_id = str(tool_id)
            if tool_id not in tool_dic_ids:
                re_time += 1
                retry_tool_id += 1
                print("Tool ID wrong! Generate tool_id that do not exist!")
                continue
            tool_des_json = tool_doc[str(tool_id)]
            retry_tool_id = 0
        except:
            retry_tool_id += 1
            print("Tool ID wrong! Generate tool_id that do not exist!")
        ...
        tool_name = tool_des_json["tool_name"]
        #只加入relevant api
        # api_name_list = []
        # for relevant_API in relevant_APIs:
        #     if relevant_API[0] == tool_name:
        #         api_name_list.append(relevant_API[1])
        tool_des = tool_des_json["tool_description"]
        API_doc = tool_des_json["tool_guidelines"]
        # API_doc = {}
        ...
        for key in API_doc.keys():
            API_list_ele = {"name": "", "description": "", "Example": {"Scenario": ""}}
            current_api = API_doc[key]
            API_list_ele["name"] = current_api["name"]
            API_list_ele["description"] = current_api["description"]
            API_list_ele["Example"]["Scenario"] = current_api["Example"]["Scenario"]
            API_list.append(API_list_ele)
        ...
        while True:
            ...
            if api == "":
                ...
                try:
                    ...
                    re_api += 1
                    re_time += 1
                    print("API generate wrong, try again!")
                    continue
                ...
                while True:
                    try:
                        ...
                        parameter = choose_parameter_depend(API_dic, query, answer_log, thought, chat)
                        if parameter == -1:
                            retry_parameter += 1
                            re_time += 1
                            continue
...
run("/home/chenjunzhi/JARVIS/easytool/data_toolbench/test_data/G2_category.json", "/home/chenjunzhi/JARVIS/easytool/data_toolbench/tool_instruction/toolbench_tool_instruction.json", "/mntcephfs/lab_data/chenjunzhi/data/toolenv/tools", "data/mistral_multi/decompose_answer_v5_g2.json", chat, "data/mistral_multi/decompose_whole_v5_g2.json") 
run("/home/chenjunzhi/JARVIS/easytool/data_toolbench/test_data/G3_instruction.json", "/home/chenjunzhi/JARVIS/easytool/data_toolbench/tool_instruction/toolbench_tool_instruction.json", "/mntcephfs/lab_data/chenjunzhi/data/toolenv/tools", "data/mistral_multi/decompose_answer_v5_g3.json", chat, "data/mistral_multi/decompose_whole_v5_g3.json") 
